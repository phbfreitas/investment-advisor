/**
 * One-time migration: encrypt all plaintext financial fields in DynamoDB.
 *
 * Usage:
 *   KMS_KEY_ID=<key-id> DYNAMODB_TABLE_NAME=<table> npx tsx scripts/migrate-encrypt.ts
 *
 * Safe to re-run — fields already starting with ENC: are skipped.
 */
...
import { encryptField, isEncrypted } from '../src/lib/encryption/crypto';
...
async function main(): Promise<void> {
  console.log(`Starting encryption migration on table: ${TABLE_NAME}`);
    ...
    for (const item of items) {
      ...
      try {
        ...
        for (const field of classification.encryptedFields) {
          ...
          updated[field] = encryptField(value, plaintextKey, aad);
          changed = true;
        }
        ...
        errors++;
        console.error(`  ERROR encrypting item PK=${pk} SK=${sk}:`, err);
      }
    }
  ...
  if (errors > 0) {
    console.error('\nWARNING: Some items failed to encrypt. Check logs above and re-run to retry.');
    process.exit(1);
  }
}
