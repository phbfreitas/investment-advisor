...
#### D. Portfolio Classification (Rules-Based Engine)
The system uses a strict, data-driven decision tree to categorize every asset into one of three strategy types. This ensures your portfolio alignment is always calculated against quantitative reality, not just marketing names.

The AI fetches the **Trailing Dividend Yield** and **3-Year Beta** from Yahoo Finance and applies the following hierarchy:

1.  **Pure Growth (Crescimento Puro)**
    *   **Logic:** Yield is between **0.0% and 2.0%**.
    *   **Validation:** The AI scans the business summary for terms like *'Index'*, *'S&P 500'*, *'Nasdaq'*, or *'Capital Appreciation'*.
    *   **Goal:** Long-term capital gains with minimal immediate income.

2.  **Pure Dividend (Dividendos Puros)**
    *   **Logic:** Yield is between **2.1% and 8.0%** AND **Beta is less than 1.0** (low volatility).
    *   **Validation:** The summary must contain keywords like *'Dividend'*, *'Distribution'*, *'Realty'*, or *'Utility'*.
    *   **Goal:** Stable, predictable cash flow from established companies/funds.

3.  **The Mix / Hybrids (O Mix)**
    *   **Path A (High Yield):** Yield is **greater than 8.0%**. The AI verifies keywords like *'Options'*, *'Covered Call'*, *'Derivative'*, or *'Distribution'* to confirm the yield is generated by a specific strategy.
    *   **Path B (Hybrid Risk):** Yield is between **2.1% and 8.0%** but **Beta is greater than 1.0**. This captures growth assets that pay dividends but carry higher market risk.
    *   **Default Fallback:** If an asset's data is uncertain or doesn't strictly fit the other categories, it defaults to "The Mix" to ensure conservative risk mapping.
