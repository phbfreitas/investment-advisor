  ...
  return (
    <div className="flex flex-col h-full bg-[#050505]">
      ...
      <header className="flex-none h-16 border-b border-neutral-800 flex items-center px-8 bg-[#0a0a0a]/80 backdrop-blur-sm sticky top-0 z-10">
        <h1 className="text-xl font-medium text-neutral-200">Value Investing Advisor</h1>
      </header>
        ...
        <div className="max-w-5xl mx-auto space-y-12 pb-32">
          ...
          {messages.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-20 text-center space-y-6 animate-in fade-in duration-700">
              <div className="h-20 w-20 rounded-full glass-panel-accent flex items-center justify-center mb-4 text-4xl shadow-inner">
                👴
              </div>
              <h2 className="text-3xl font-semibold text-neutral-100">Welcome to your Buffett Advisor</h2>
              ...
              <div className="grid grid-cols-2 gap-4 mt-8 w-full max-w-xl">
                ...
                >
                  <BarChart2 className="h-5 w-5 text-teal-500 group-hover:scale-110 transition-transform" />
                ...
                <button
                  ...
                  className="flex items-center space-x-3 p-4 glass-panel hover:bg-neutral-800 transition-colors text-left group"
                >
                  <BrainCircuit className="h-5 w-5 text-indigo-400 group-hover:scale-110 transition-transform" />
            ...
            <div className="space-y-12">
              {messages.map((msg) => (
                <div key={msg.id} className={`flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
                  {msg.role === 'user' ? (
                    <div className="bg-neutral-800 text-neutral-100 px-6 py-4 rounded-2xl rounded-tr-sm max-w-2xl text-lg shadow-md break-words">
                      {msg.content}
                    </div>
                  ) : (
              ...
              {isLoading && (
                <div className="flex items-center space-x-4 text-teal-500 bg-teal-500/5 px-6 py-4 rounded-2xl w-fit border border-teal-500/10 animate-pulse">
                  <RefreshCw className="h-5 w-5 animate-spin" />
                  <span className="font-medium tracking-wide">Warren is reviewing his letters and analyzing your portfolio...</span>
                </div>
              )}
      ...
      {/* Input Area */}
      <div className="flex-none p-6 bg-gradient-to-t from-[#050505] via-[#050505] to-transparent">
        <form onSubmit={handleSubmit} className="max-w-4xl mx-auto">
          <div className="glass-panel p-2 flex items-end relative shadow-2xl shadow-teal-900/10 focus-within:ring-1 focus-within:ring-teal-500/50 transition-all">
            <textarea
              ...
              className="flex-1 max-h-48 min-h-[56px] w-full resize-none bg-transparent px-4 py-4 text-base text-neutral-100 placeholder:text-neutral-500 focus:outline-none focus:ring-0 custom-scrollbar"
              placeholder="Ask Warren a question... (Press Enter to send)"
              rows={1}
              disabled={isLoading}
            />
            <div className="absolute right-4 bottom-4 flex items-center space-x-2">
              <button
                ...
                className="h-10 w-10 rounded-xl bg-teal-500/10 text-teal-400 flex items-center justify-center hover:bg-teal-500 hover:text-white transition-all disabled:opacity-50 disabled:hover:bg-teal-500/10 disabled:hover:text-teal-400"
              >
